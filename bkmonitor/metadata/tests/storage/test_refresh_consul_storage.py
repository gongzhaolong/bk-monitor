...
class MockHashConsul:
    ...
    def get(self, key):
        """模拟 Consul get 方法，返回 (index, value_dict) 格式"""
        for item in self.result_list:
            if item["key"] == key:
                return (None, {"Value": json.dumps(item["value"])})
        return (None, None)
...
class TestRefreshConsulStorage:
    ...
    def test_refresh_consul_storage_config_single_cluster(self, mock_consul, mocker):
        ...
        assert len(mock_consul.result_list) >= 2  # 至少有一个配置和一个版本信息

        # 查找对应的配置项
        config_item = None
        for item in mock_consul.result_list:
            if item["key"] == consul_path:
                config_item = item
                break
        ...
        # 验证配置内容正确
        stored_config = config_item["value"]
        ...
        version_item = None
        for item in mock_consul.result_list:
            if item["key"] == ClusterInfo.CONSUL_VERSION_PATH:
                version_item = item
                break
        assert version_item is not None
        assert "time" in version_item["value"]
    ...
    def test_refresh_consul_storage_config_multiple_clusters(self, mock_consul, mocker):
        ...
        # 验证版本信息只写入一次
        version_count = sum(1 for item in mock_consul.result_list if item["key"] == ClusterInfo.CONSUL_VERSION_PATH)
    ...
    def test_refresh_consul_storage_config_consul_path_format(self, mock_consul, mocker):
        ...
        config_item = None
        for item in mock_consul.result_list:
            if item["key"] == expected_path:
                config_item = item
                break

        assert config_item is not None
        assert config_item["key"] == expected_path
    ...
    def test_refresh_consul_storage_config_version_info(self, mock_consul, mocker):
        ...
        assert version_item is not None
        assert "time" in version_item["value"]
        version_time = version_item["value"]["time"]
    ...
    def test_refresh_consul_storage_config_empty_cluster_list(self, mock_consul, mocker):
        ...
        # 验证只写入了版本信息，没有配置信息
        config_items = [item for item in mock_consul.result_list if item["key"] != ClusterInfo.CONSUL_VERSION_PATH]
        assert len(config_items) == 0

        # 验证版本信息已写入
        version_items = [item for item in mock_consul.result_list if item["key"] == ClusterInfo.CONSUL_VERSION_PATH]
    ...
    def test_refresh_consul_storage_config_duplicate_cluster_id(self, mock_consul, mocker):
        ...
        consul_path = "/".join([ClusterInfo.CONSUL_PREFIX_PATH, "1"])
        config_items = [item for item in mock_consul.result_list if item["key"] == consul_path]
        assert len(config_items) == 1

        # 验证写入的是最后一个配置
        stored_config = config_items[0]["value"]
